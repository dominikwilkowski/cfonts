...
pub static FONT_NEAT: FontFile<6> = FontFile {
	name: "neat",
	#[rustfmt::skip]
	buffer_start: &[
...
mod tests {
	...
	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_NEAT);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_NEAT);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_NEAT);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_NEAT);
	}

	#[test]
	fn buffers_complementary() {
		assert_buffers_complementary(&super::FONT_NEAT);
	}
}
