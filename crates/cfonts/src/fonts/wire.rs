...
pub static FONT_WIRE: FontFile<5> = FontFile {
	name: "wire",
	#[rustfmt::skip]
	buffer_start: &[
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_WIRE);
	}

	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_WIRE);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_WIRE);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_WIRE);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_WIRE);
	}

	#[test]
	fn buffers_complementary() {
		assert_buffers_complementary(&super::FONT_WIRE);
	}
}
