...
pub static FONT_VISION: FontFile<3> = FontFile {
	name: "vision",
	#[rustfmt::skip]
	buffer_start: &[
	...
	glyphs: {
		...
		table['A' as usize] = Some(glyph!(
			r"<c1>╓──┐</c1><c2>┐</c2><c3>┐</c3>",
			r"<c1>╟──┤</c1><c2>┤</c2><c3>┤</c3>",
			r"<c1>╨</c1><c2>╨</c2><c3>╨</c3><c1>┴</c1><c2>┴</c2><c3>┴</c3>",
		));
		...
		table['K' as usize] = Some(glyph!(
			r"<c1>╥</c1><c2>╥</c2><c1>┌</c1><c2>┌</c2><c3>┌ </c3>",
			r"<c1>╟─┴┐</c1><c2>┐</c2><c3>┐</c3>",
			r"<c1>╨</c1><c2>╨</c2><c3>╨</c3><c1>┴</c1><c2>┴</c2><c3>┴</c3>",
		));
		...
		table['M' as usize] = Some(glyph!(
			r"<c1>╓─╥─┐</c1><c2>┐</c2><c3>┐</c3>",
			r"<c1>║</c1><c2>║</c2><c1>║</c1><c2>║</c2><c1>│</c1><c2>│</c2><c3>│</c3>",
			r"<c1>╨</c1><c2>╨</c2><c1>╨</c1><c2>╨</c2><c1>┴</c1><c2>┴</c2><c3>┴</c3>",
		));
		...
		table['%' as usize] = Some(glyph!(
			r"<c1>o</c1><c2>o</c2><c1>┌</c1><c2>┌</c2><c3>┌</c3>",
			r"<c1> ┌┘</c1><c2>┘</c2><c3>┘</c3>",
			r"<c1> ┘</c1><c2>┘</c2><c1>o</c1><c2>o</c2>",
		));
		table['&' as usize] = Some(glyph!(
			r"<c1> │</c1><c2>│</c2><c3>│ </c3>",
			r"<c1>┌┼─</c1><c2>─</c2><c3>─</c3>",
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_VISION);
	}

	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_VISION);
	}

	#[test]
	fn plain_segments_are_spaces() {
		assert_plain_segments_are_spaces(&super::FONT_VISION);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_VISION);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_VISION);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_VISION);
	}

	#[test]
	fn buffers_complementary() {
		assert_buffers_complementary(&super::FONT_VISION);
	}
}
