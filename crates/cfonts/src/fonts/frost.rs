...
pub static FONT_FROST: FontFile<5> = FontFile {
	name: "frost",
	#[rustfmt::skip]
	buffer_start: &[
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_FROST);
	}

	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_FROST);
	}

	#[test]
	fn plain_segments_are_spaces() {
		assert_plain_segments_are_spaces(&super::FONT_FROST);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_FROST);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_FROST);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_FROST);
	}

	#[test]
	fn buffers_complementary() {
		assert_buffers_complementary(&super::FONT_FROST);
	}

	#[test]
	fn slots_paint_only_their_material() {
		assert_slots_paint_only(&super::FONT_FROST, &[&['░'], &['▌']]);
	}
}
