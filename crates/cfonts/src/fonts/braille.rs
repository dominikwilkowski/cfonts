...
pub static FONT_BRAILLE: FontFile<2> = FontFile {
	name: "braille",
	#[rustfmt::skip]
	buffer_start: &[
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_BRAILLE);
	}
	...
	fn rows_stripe_their_slot() {
		assert_rows_stripe_their_slot(&super::FONT_BRAILLE);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_BRAILLE);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_BRAILLE);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_BRAILLE);
	}

	#[test]
	fn buffers_complementary() {
		assert_buffers_complementary(&super::FONT_BRAILLE);
	}
}
