...
pub static FONT_DEPTH: FontFile<5> = FontFile {
	name: "depth",
	#[rustfmt::skip]
	buffer_start: &[
	...
	glyphs: {
		...
		table['B' as usize] = Some(glyph!(
			r"<c2> /</c2><c1>█▀▀█ </c1>",
			r"<c2>│ </c2><c1>▓▓▓▓▓</c1>",
			r"<c2>│ </c2><c1>▒</c1><c2>__/</c2><c1>▒</c1>",
			r"<c2>│ </c2><c1>░░░░░</c1>",
			r"<c2>│/____/</c2>",
		));
		table['C' as usize] = Some(glyph!(
			r"<c2> /</c2><c1>████</c1>",
			r"<c2>│ </c2><c1>▓</c1><c2>__/</c2>",
			r"<c2>│ </c2><c1>▒   </c1>",
			r"<c2>│ </c2><c1>░░░░</c1>",
			r"<c2>│/___/</c2>",
		));
		...
		table['&' as usize] = Some(glyph!(
			r"<c2>    /</c2><c1>██  </c1>",
			r"<c2> /</c2><c1>▓▓▓▓▓▓▓</c1>",
			r"<c2>│//</c2><c1>▒</c1><c2>/</c2><c1>▒▒</c1><c2>_/</c2>",
			r"<c2> │//</c2><c1>░░░  </c1>",
			r"<c2>  │/__/  </c2>",
		));
		table['(' as usize] = Some(glyph!(
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_DEPTH);
	}

	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_DEPTH);
	}

	#[test]
	fn plain_segments_are_spaces() {
		assert_plain_segments_are_spaces(&super::FONT_DEPTH);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_DEPTH);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_DEPTH);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_DEPTH);
	}
