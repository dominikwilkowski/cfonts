...
pub static FONT_THIN: FontFile<3> = FontFile {
	name: "thin",
	#[rustfmt::skip]
	buffer_start: &[
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_THIN);
	}

	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_THIN);
	}

	#[test]
	fn plain_segments_are_spaces() {
		assert_plain_segments_are_spaces(&super::FONT_THIN);
	}

	#[test]
	fn rows_stripe_their_slot() {
		assert_rows_stripe_their_slot(&super::FONT_THIN);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_THIN);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_THIN);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_THIN);
	}

	#[test]
	fn buffers_complementary() {
		assert_buffers_complementary(&super::FONT_THIN);
	}
}
