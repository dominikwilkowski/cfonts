...
pub static FONT_BRIDGE: FontFile<3> = FontFile {
	name: "bridge",
	#[rustfmt::skip]
	buffer_start: &[
	...
	glyphs: {
		...
		table['#' as usize] = Some(glyph!(
			r"<c2>_</c2><c1>▄</c1><c2>_</c2><c1>▄</c1><c2>_</c2>",
			r"<c2>_</c2><c1>█</c1><c2>_</c2><c1>█</c1><c2>_</c2>",
			r"<c1> ▀ ▀ </c1>",
		));
		table['$' as usize] = Some(glyph!(
...
mod tests {
	...
	fn all_supported_glyphs_defined() {
		assert_supported(&super::FONT_BRIDGE);
	}

	#[test]
	fn colors_all_used() {
		assert_colors_all_used(&super::FONT_BRIDGE);
	}

	#[test]
	fn plain_segments_are_spaces() {
		assert_plain_segments_are_spaces(&super::FONT_BRIDGE);
	}

	#[test]
	fn buffer_start_size() {
		assert_buffer_start_size(&super::FONT_BRIDGE);
	}

	#[test]
	fn buffer_end_size() {
		assert_buffer_end_size(&super::FONT_BRIDGE);
	}

	#[test]
	fn buffers_plain() {
		assert_buffers_plain(&super::FONT_BRIDGE);
	}
