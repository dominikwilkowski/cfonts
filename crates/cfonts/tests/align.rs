...
fn cli_center_alignment_floors_odd_padding() {
	...
	with_force_size(8, || {
		let rendered = Cfonts::text("A").font(Font::Tiny).align(Align::Center).spaceless().render(&RustHost::default());
...
fn cli_alignment_adds_no_padding_when_the_row_already_fills_the_canvas() {
	// Fits just right
	with_force_size(3, || {
		let rendered = Cfonts::text("A").font(Font::Tiny).align(Align::Right).spaceless().render(&RustHost::default());
...
fn cli_does_not_pad_empty_line_height_rows() {
	...
	with_force_size(7, || {
		let rendered =
			Cfonts::text("A|B").font(Font::Tiny).line_height(3).align(Align::Center).spaceless().render(&RustHost::default());
