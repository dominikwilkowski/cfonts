...
use cfonts::{Align, BrowserEnv, Cfonts, CliEnv, Environment, Font, Options, Valign};
...
fn cli_aligns_left_center_and_right() {
	...
	with_force_size(7, || {
		let left = Cfonts::text("A").font(Font::Tiny).align(Align::Left).spaceless().render(&CliEnv::default());
		let center = Cfonts::text("A").font(Font::Tiny).align(Align::Center).spaceless().render(&CliEnv::default());
		let right = Cfonts::text("A").font(Font::Tiny).align(Align::Right).spaceless().render(&CliEnv::default());
...
fn cli_center_alignment_floors_odd_padding() {
	...
	with_force_size(8, || {
		let rendered = Cfonts::text("A").font(Font::Tiny).align(Align::Center).spaceless().render(&CliEnv::default());
...
fn cli_alignment_adds_no_padding_when_the_row_already_fills_the_canvas() {
	// Fits just right
	with_force_size(3, || {
		let rendered = Cfonts::text("A").font(Font::Tiny).align(Align::Right).spaceless().render(&CliEnv::default());
...
fn cli_aligns_each_rendered_line_by_its_own_width() {
	...
	with_force_size(11, || {
		let rendered =
			Cfonts::text("A|BB").font(Font::Tiny).line_height(0).align(Align::Center).spaceless().render(&CliEnv::default());
...
fn cli_does_not_pad_empty_line_height_rows() {
	...
	with_force_size(7, || {
		let rendered =
			Cfonts::text("A|B").font(Font::Tiny).line_height(3).align(Align::Center).spaceless().render(&CliEnv::default());
...
fn cli_aligns_wrapped_lines_by_their_own_width() {
	...
	with_force_size(9, || {
		let rendered =
			Cfonts::text("AA BB").font(Font::Tiny).line_height(0).align(Align::Center).spaceless().render(&CliEnv::default());
...
fn browser_wrapper_carries_each_alignment() {
	...
	without_force_size(|| {
		for (align, css) in [
			...
			let rendered = Cfonts::text("HI").font(Font::Block).align(align).render(&BrowserEnv);
...
fn the_default_alignment_is_left() {
	without_force_size(|| {
		let rendered = Cfonts::text("A").font(Font::Tiny).render(&BrowserEnv);
...
fn browser_alignment_is_pure_css_and_does_not_touch_the_rows() {
	...
	without_force_size(|| {
		for font in ALL_FONTS {
			let left = Cfonts::text("HI").font(*font).align(Align::Left).render(&BrowserEnv);
			let center = Cfonts::text("HI").font(*font).align(Align::Center).render(&BrowserEnv);
			let right = Cfonts::text("HI").font(*font).align(Align::Right).render(&BrowserEnv);
...
fn browser_alignment_applies_to_multi_font_compositions() {
	...
	without_force_size(|| {
		let rendered =
			Cfonts::text("HI").font(Font::Block).new_text("THERE").font(Font::Tiny).align(Align::Center).render(&BrowserEnv);
...
fn empty_text_still_renders_an_aligned_wrapper() {
	...
	without_force_size(|| {
		let rendered = Cfonts::text("").font(Font::Block).align(Align::Center).render(&BrowserEnv);
		assert_eq!(rendered.text.matches("text-align:center").count(), 1);
	});
}
...
fn tweaked_options_render_with_their_alignment() {
	...
	without_force_size(|| {
		...
		assert_eq!(BrowserEnv.render_from(&options).text.matches("text-align:center").count(), 1);
	});
}
...
fn forced_wrapping_keeps_one_alignment_wrapper() {
	...
	with_force_size(3, || {
		let rendered =
			Cfonts::text("AA").font(Font::Tiny).line_height(0).align(Align::Center).spaceless().render(&BrowserEnv);
...
fn alignment_does_not_change_wrapped_browser_rows() {
	...
	with_force_size(3, || {
		let left = Cfonts::text("AA").font(Font::Tiny).align(Align::Left).render(&BrowserEnv);
		let center = Cfonts::text("AA").font(Font::Tiny).align(Align::Center).render(&BrowserEnv);
		let right = Cfonts::text("AA").font(Font::Tiny).align(Align::Right).render(&BrowserEnv);
...
fn align_is_global_ignores_setter_position() {
	...
	without_force_size(|| {
		let rendered1 =
			Cfonts::text("A").font(Font::Tiny).new_text("B").font(Font::Block).align(Align::Right).render(&BrowserEnv);
		let rendered2 =
			Cfonts::text("A").font(Font::Tiny).new_text("B").align(Align::Right).font(Font::Block).render(&BrowserEnv);
		let rendered3 =
			Cfonts::text("A").font(Font::Tiny).align(Align::Right).new_text("B").font(Font::Block).render(&BrowserEnv);
		let rendered4 =
			Cfonts::text("A").align(Align::Right).font(Font::Tiny).new_text("B").font(Font::Block).render(&BrowserEnv);
...
fn no_blocks_still_render_with_their_alignment() {
	...
	without_force_size(|| {
		...
		let rendered = BrowserEnv.render_from(&options);
...
fn browser_wrapper_contains_exactly_one_text_align_declaration() {
	...
	without_force_size(|| {
		let rendered = Cfonts::text("Hi").font(Font::Tiny).align(Align::Center).render(&BrowserEnv);
